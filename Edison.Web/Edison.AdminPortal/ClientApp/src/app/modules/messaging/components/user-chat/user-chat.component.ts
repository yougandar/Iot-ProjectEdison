...
import { SelectActiveUser, SendNewMessage, EndConversation } from '../../../../reducers/chat/chat.actions';
...
export class UserChatComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.messagesSub$ = this.store.select(chatActiveMessagesSelector).subscribe(messages => {
            this.messages = messages;
        });
        this.activeUserId$ = this.store.select(chatActiveUserSelector).subscribe(user => {
            this.userId = user.userId;
            this.userName = user.name;
        });
        this.actionPlans$ = this.store.select(actionPlansSelector);
    }
