...
export class EventBarComponent {
  ...
  constructor(private store: Store<AppState>) {
    store.pipe(select(eventsSelector)).subscribe(events => {
      this.onEventsUpdate(events);
    });

    store.pipe(select(responsesSelector)).subscribe(responses => {
  ...
  onEventsUpdate = (events: Event[]) => {
    ...
    this.recentEventsLazy = events.filter(
      (v, i) => i > 0 && i < this.lazyCount
    );
  ...
  getEventCount() {
    return this.allEvents.filter(e => e.closureDate === null).reduce((a, v) => (a += v.eventCount), 0);
  }
