...
import { ShowEvents, SelectActiveEvent } from '../../../../reducers/event/event.actions';
...
export class EventCardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.store.pipe(select(responsesSelector)).subscribe(responses => {
      this.response = responses.find(response => response.primaryEventClusterId === this.event.eventClusterId);
    });
  }
  ...
  activateResponse = () => {
    this.activeEventSub = this.store.pipe(select(activeEventSelector)).subscribe(activeEvent => {
      if (activeEvent && activeEvent.eventClusterId === this.event.eventClusterId) {
        this.active = true;
