...
import { SelectActiveResponse, GetResponse } from '../../../../reducers/response/response.actions';
...
export class ActiveResponsesComponent implements OnInit {
  active = false;
  ...
  ngOnInit() {
    this.store.pipe(select(activeResponsesSelector))
    .subscribe(responses => {
      this.activeResponses = responses;

      let responsesToShow = responses;
      if (this.activeResponse) {
        const activeResponseResolved = !responsesToShow.some(response => response.responseId === this.activeResponse.responseId);
        if (activeResponseResolved) {
          // we want to let this response still show until the active responses window closes
          responsesToShow = [...responses, {
            ...this.activeResponse,
